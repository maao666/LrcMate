...
class NeteaseSearch():
    ...
    def __init__(self, keyword, search_type = 1, offset = 0, limit = 1):
        search_url = 'http://music.163.com/api/search/get'
        data = {
            's': keyword, # S stands for keyword to search
            'type': search_type, # 1 for songs, 10 for albums, 100 for artists, 1000 for playlists, 1002 for users
            'offset': offset, # For splitting pages
            'limit': limit # Item quantity to return 
            #'MUSIC_U' Meaning unknown
        }

        self.raw_response = requests.post(search_url, data, headers = self.headers)
        try:
            logging.debug('response text with type {0}: {1}'.format(type(self.raw_response.text),self.raw_response.text))
        except:
            pass

        self.search_result_dict = json.loads(self.raw_response.text)
        try:
            logging.debug('json result with type {0}: {1}'.format(type(self.search_result_dict),self.search_result_dict))
        except:
            pass
        ...
        if self.search_result_dict['result']['songCount'] == 0:
            ...
            try:
                logging.critical('Unable to find song with keyword {}'.format(keyword))
            except:
                pass
        else:
    ...
    def _fetch_lrc(self):
        ...
        lyrics_url = 'http://music.163.com/api/song/lyric?id={0}&lv=-1&kv=-1&tv=-1'.format(individual_id)
        ...
        try:
            self.translater_username = self.lyrics_result['transUser']['nickname']
        except:
            self.lyrics_username = None
        # Get lyrics
        try:
